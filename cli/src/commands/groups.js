...
export function registerGroupsCommands(program) {
  ...
  grp
    ...
    .action(async (opts) => {
      ...
      const { groups } = await getCategoryGroups(db);
  ...
  grp
    ...
    .action(async (name, opts) => {
      ...
      const cats = await getCategoriesList(db);
      const union = [...new Set([...cats, ...groups[name]])].sort();
      await db.collection('config').doc('categories').set({ list: union });
  ...
  grp
    ...
    .action(async (name, opts) => {
      ...
      const allCats = new Set(await getCategoriesList(db));
      Object.values(groups).flat().forEach((c) => allCats.add(c));
      await db.collection('config').doc('categories').set({ list: [...allCats].sort() });
